...
const Time = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-16">
      ...
      <p className="text-3xl text-[#facf2e] mt-3 text-center">
        JOIN US IN WORSHIP
      </p>
      <p className="leading-5 mt-3 text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-5">
        {features.map((item) => (
          <div key={item.id} className="p-6 bg-white rounded-2xl shadow-sm">
            ...
            <h3 className="text-sm font-bold text-[#facf2e] mb-3">
              {item.subtitle}
            </h3>

            <p className="text-gray-600 font-[15px]">{item.text}</p>
          </div>
        ))}
      </div>
    </div>
